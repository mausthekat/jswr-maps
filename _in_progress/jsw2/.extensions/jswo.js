...
/**
 * JSW:R Tiled Extensions
 * - New Room: Creates a new empty room for the JSW Tiled map project
 * - Snap to Exit: Moves current room in world to align with its exit connections
 * - Go to Exit: Opens the room connected via exit in specified direction
 *
 * Adds items to Map menu
 */
    ...
    try {
        ...
        while ((groupMatch = objectGroupRegex.exec(content)) !== null) {
            ...
            while ((objMatch = objectRegex.exec(groupContent)) !== null) {
                ...
                const gidMatch = attrs.match(/gid="(\d+)"/);
                const hasGid = gidMatch !== null;
...
function fixSelectedGuardianProperties() {
    ...
    // Build report
    const report = [];
    let totalFixed = fixedCount + orphanFixedCount;
...
const fixSelectedAction = tiled.registerAction("JSWRFixSelected", fixSelectedGuardianProperties);
...
// Add to New menu too (useful if no map open).
