...
def _cluster_4connected(cells: set[tuple[int, int]]) -> list[list[tuple[int, int]]]:
    """4-connected (cardinal-neighbour) clustering of (col, row) cells."""
    rest = set(cells)
...
def classify_room_contents(im: Image.Image, grid: Grid,
    ...
    Clustering key is **attribute pair (paper, ink) only** — bitmap is
    too volatile (pattern variations within structural tile-types make
    every cell unique). For each attribute pair we then look at how its
    cells are spatially distributed:

      * many tiles, in long horizontal/vertical runs or large blobs ->
        structural (background, floor, wall, ramp, conveyor, ladders)
      * few isolated single cells -> items
      * one or two 2x2-ish clusters -> guardians
    ...
    for i, rr in enumerate(room_regions):
        ...
        # For each attribute pair, decide if it's structural based on
        # spatial distribution. Rules:
        #   * any cluster spanning >= 4 cells in either dimension OR
        #     >= 8 cells total -> structural (long runs, big blobs)
        #   * few isolated 1- or 2-cell clusters -> sprite candidate
        attr_clusters: dict[tuple[int, int], list[list[tuple[int, int]]]] = {}
        structural: set[tuple[int, int]] = set()
        for ap, cells in attr_cells.items():
            clusters = _cluster_4connected(set(cells))
            attr_clusters[ap] = clusters
            is_structural = False
            for cluster in clusters:
                xs = {cx for cx, _ in cluster}
                ys = {cy for _, cy in cluster}
                span = max(len(xs), len(ys))
                if span >= 4 or len(cluster) >= 8:
                    is_structural = True
                    break
            if is_structural:
                structural.add(ap)
        ...
        for ap, clusters in attr_clusters.items():
            if ap in structural:
                continue
            paper, ink = ap
            # Skip pure black-on-black (empty bg cells should never reach
            # here, but guard anyway).
            if paper == 0 and ink == 0:
                continue
            for cluster in clusters:
                shape_sc, sprite_type = _shape_score(len(cluster))
                # Rarity is per-cluster: a 1-cell cluster of an
                # attribute that occurs 5 times across the room is still
                # an isolated occurrence locally. Use cluster-internal
                # rarity (smaller cluster = rarer locally).
                rarity = _rarity_score(len(cluster))
                sat = _saturation_score(ink, paper)
                conf = min(1.0,
                           0.40 * rarity + 0.35 * sat + 0.25 * shape_sc)
